...
- Azure DevOps integration:
  - OAuth 2.0 (Authorization Code with PKCE) via Microsoft Entra ID
  - One organisation per settings profile; select a single project for sync scope
...
- Enter your **Organisation name** (the URL segment from `https://dev.azure.com/your-org`)
...
- To switch organisations, disconnect and reconnect with a different organisation name

#### Entra app registration (Azure DevOps)

1. Register an application in [Microsoft Entra admin center](https://entra.microsoft.com/)
2. Platform: **Mobile and desktop applications** with redirect URI `http://localhost` (loopback; ephemeral ports are accepted)
3. API permissions: **Azure DevOps** → delegated `user_impersonation` (or grant admin consent for your tenant)
4. Copy the Application (client) ID into `AZURE_DEVOPS_CLIENT_ID_DEV` (local builds) or `AZURE_DEVOPS_CLIENT_ID_PROD` (production builds)
...
Development builds require `GOOGLE_TASKS_CLIENT_ID_DEV` (see `.env.example`). Optionally set `OUTLOOK_CLIENT_ID_DEV` to enable Outlook Connect and `AZURE_DEVOPS_CLIENT_ID_DEV` to enable Azure DevOps Connect in local builds.
...
2. Build production: `GOOGLE_TASKS_CLIENT_ID_PROD=your-id OUTLOOK_CLIENT_ID_PROD=your-id npm run build:prod` (Google Tasks and Outlook client IDs required; optionally add `AZURE_DEVOPS_CLIENT_ID_PROD=your-id` for Azure DevOps Connect)
