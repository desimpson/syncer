...
- Azure DevOps: Auth Code + PKCE via Entra ID; Azure DevOps resource scope (`49939cec-5e97-4525-bb00-e3888f55c1a1/.default`); register **Mobile and desktop** redirect URIs for `http://localhost`; personal Microsoft accounts use `common` authority (not `consumers`) because Azure DevOps scopes are not exposed on `consumers`
