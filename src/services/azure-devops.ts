...
export const fetchProjects = async (
  ...
  const { status, text } = await azureDevOpsRequest(url, accessToken);

  if (status < 200 || status >= 300) {
    throwResponseError("list projects", status, text);
  }

  const page = azureDevOpsProjectsPageSchema.parse(JSON.parse(text) as unknown);
  return page.value;
};
...
const queryAssignedWorkItemIds = async (
  ...
  const parsed = wiqlResponseSchema.parse(JSON.parse(text) as unknown);
...
const fetchWorkItemDetailsChunk = async (
  ...
  const batch = workItemsBatchSchema.parse(JSON.parse(text) as unknown);
...
export const fetchAssignedWorkItems = async (
  ...
  const ids = await queryAssignedWorkItemIds(accessToken, organization, projectName);
  if (ids.length === 0) {
    return [];
  }
  ...
  for (const chunk of chunks) {
    try {
      const items = await fetchWorkItemDetailsChunk(accessToken, organization, chunk);
      results.push(...items);
    } catch (error) {
      console.warn(
